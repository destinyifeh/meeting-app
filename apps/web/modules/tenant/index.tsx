"use client";
import { Authorization, ROLES } from "@lib/hooks/useAuthorization";
import * as DropdownMenu from "@radix-ui/react-dropdown-menu";
import { Button, showToast, Table } from "@vms/ui";
...
import { useResendActivationLink } from "./api/resend-activate-link";
          ...
          <Table<TenantTableProps & { idx: string }>
            ...
            columns={[
              ...
              { title: "Status", field: "status" },
              {
                title: "Action",
                field: "id",
                Cell: ({ entry }) => <MoreMenu tenantId={entry.id} />,
              },
            ]}
            data={
            ...
            totalItems={tenants.data?.totalCount}
            pageSize={filter?.pageSize}
...
const MoreMenu = ({ tenantId }: { tenantId: string }) => {
  const onSuccess = (response: ServerResponse<Record<string, string>>) => {
    if (response.data) {
      showToast("Activation link sent successfully", "success");
    } else {
      showToast(response.error?.message as string, "error");
    }
  };
  const resendActivation = useResendActivationLink({
    onSuccess,
  });
  return (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        ...
        >
          ...
          <DropdownMenu.Item
            onClick={() => resendActivation.mutate(tenantId)}
            className=" flex  gap-3 group text-[13px] cursor-pointer leading-none text-violet11 rounded-[3px] items-center h-[25px] px-[5px] relative  select-none outline-none data-[disabled]:text-mauve8 data-[disabled]:pointer-events-none data-[highlighted]:bg-violet9 data-[highlighted]:text-violet1"
          >
            <span>
